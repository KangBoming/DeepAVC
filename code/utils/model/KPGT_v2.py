...
# KPGT model configuration
config_dict = {
    'base': {
        'd_node_feats': 137, 'd_edge_feats': 14, 'd_g_feats': 768, 'd_hpath_ratio': 12, 
        'n_mol_layers': 12, 'path_length': 5, 'n_heads': 12, 'n_ffn_dense_layers': 2,'input_drop':0.0, 
        'attn_drop': 0.1, 'feat_drop': 0.1, 'batch_size': 1024, 'lr': 2e-04, 'weight_decay': 1e-6,
...
class LiGhTPredictor(nn.Module):
    ...
    def generate_node_feature(self, g, fp, md):
        # 获取指示器，用于区分正常节点、虚拟节点等
        indicators = g.ndata['vavn']  # 0表示正常节点，-1表示虚拟原子，>=1表示虚拟节点
        # 输入部分
        node_h = self.node_emb(g.ndata['begin_end'], indicators)          
        edge_h = self.edge_emb(g.ndata['edge'], indicators)
        triplet_h = self.triplet_emb(node_h, edge_h, fp, md, indicators)
        ...
        # 使用 torch.split() 根据长度进行分割
        node_tensor_list = torch.split(triplet_h, lengths.tolist())

        ### 对不等长的序列进行padding
        smiles_tensor_padded = pad_sequence(sequences=node_tensor_list,
                                        batch_first=True,
                                        padding_value=0)
        

        smiles_padding_mask = (smiles_tensor_padded == 0).all(dim=-1)
        ...
        return smiles_tensor_padded, smiles_padding_mask,g_feats
